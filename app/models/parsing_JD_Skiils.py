...
def extract_skills(jd_text: str) -> List[str]: 
    ...
    - Programming languages (expand all abbreviations. For example, if given "JS" expand it to "JavaScript")
    - Frameworks/libraries (For example, if given "TF" expand it to "TensorFlow")
    ...
    2. Ungroup similar technologies (e.g., "AWS/Azure/GCP" → "AWS, Azure, Google Cloud Platform")
    3. Standardize terms (use "React.js" not "ReactJS")
    4. Exclude non-technical skills and generic terms
    5. Remove duplicates but keep variations with different specificity
    6. Maintain original capitalization for proper nouns
    ...
    Javascript, .Net, MongoBD, ReactJS, NodeJS, etc.
    Input Text:
    {jd_text}
    """
